...
#### Number of Women under 20 in need of accessing publicly-funded contraceptives
...
#### Total Number of Women in need of accessing publicly-funded contraceptives
...
# make color palette
bins <- c("hostile", "leans_hostile", "middle_ground", "leans_supportive", "supportive")
hostility_pal <- colorBin(
  palette = "inferno",
  domain = data_sf$hostility,
  reverse = TRUE,
  bins = bins)

# this method does not work - to create a map for hostility we would need to either:
# 1) make hostility into a factor variable rather than character (probably would need 
#    to do in original merge_data.csv file because mutate behaves strangely with spatial file)
# 2) manually assign each grade to a specific color
...
hostility_map = base_map %>% 
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto")) %>% 
  addLegend(pal = hostility_pal, values = ~hostility, opacity = 0.7, title = 'State hostility ranking in 2000 according to Guttmacher ',
  position = "bottomright")
